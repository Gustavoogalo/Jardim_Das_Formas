...
namespace Helper.EventBusFolder
{
    public static class EventBus
    {
        ...
        public static void Subscribe<T>(Action<T> handler) where T : IEvents
        {
            Type t = typeof(T);
        ...
        public static void Unsubscribe<T>(Action<T> handler) where T : IEvents
        {
            Type t = typeof(T);
            if (subs.TryGetValue(t, out List<Delegate> list))
            {
        ...
        public static void Publish<T>(T ev) where T : IEvents
        {
            Type t = typeof(T);
            if (subs.TryGetValue(t, out List<Delegate> list))
            {
