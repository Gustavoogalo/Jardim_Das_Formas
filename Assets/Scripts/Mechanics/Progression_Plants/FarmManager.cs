...
using Helper.EventBusFolder; // Necessário para List<T>

public class FarmManager : MonoBehaviour
{
    [Header("Id Reference")] [SerializeField]
    private int farmId;

    [SerializeField] private int requiredStars = 9;

    [Header("Planting Settings")] [Tooltip("Todos os GameObjects vazios que representam os locais de plantio.")]
    ...
    private bool isLastFarm;
    
    public bool IsLastFarm => isLastFarm;

    public void SetIsLastFarm(bool value)
    {
        isLastFarm = value;
    }
    ...
    public void HandleStarReward(OnChallengeCompleted challengeCompletedEvent)
    {
        if (!unlocked) return;

        for (int star = 0; star < challengeCompletedEvent.StarsGained; star++)
        {
    ...
    void OnEnable()
    {
        EventBus.Subscribe<OnChallengeCompleted>(HandleStarReward);
        
    }

    void OnDisable()
    {
        EventBus.Unsubscribe<OnChallengeCompleted>(HandleStarReward);
    }
